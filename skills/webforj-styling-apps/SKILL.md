...
description: "Styles and themes webforJ applications using the DWC design-token system (--dwc-* CSS custom properties). Covers DWC v1 (webforJ 25 and prior) and DWC v2 (webforJ 26+), with version-aware routing through the webforj-mcp tools that act as the canonical token catalog. Use whenever the user writes CSS for a webforJ app, picks --dwc-* tokens, styles a DWC component, builds an app theme, configures palettes, customizes shadow parts (::part()), reskins a brand color, or when reviews flag hardcoded colors, sizes, or spacing. Always resolve the target webforJ version first, then call get_versions, styles_list_tokens, styles_get_component, styles_validate_tokens, or create_theme as needed. Differences between DWC v1 and v2 (palette config tokens, semantic variations, border-radius scale, transition defaults, focus-ring tokens) are spelled out so the skill is correct on both."
---
